...
<html>
    ...
    <head>
        ...
        <link rel="stylesheet" href="br12.css" />
...
commands.getstatusoutput("sudo nmap -sP 192.168.43.117-255 -n | grep 'Nmap scan' | awk '{print $5}' > /etc/cluster/iplist.txt")
fp = open("/etc/cluster/iplist.txt","r+")
ip=fp.read()
s=ip.split("\n")
...
while (l<len(s)-1):
	...
	out1=commands.getstatusoutput("sudo sshpass -p 123 ssh {} cat /proc/meminfo | grep 'MemTotal'| cut -d ' ' -f9".format(s[l]))

	mem[s[l]]=out1[1]
	l=l+1

sorted(mem)
print "----------------here is the list of available IPs and there RAM status------------<br><br>"
for l in mem:
	print "IP - "+l+"      RAM - "+mem[l]+"<br>"
print "<br><hr>"
#-----------------------------------Configuring Namenode-----------------------
key,value=mem.popitem()
nip=key
...
print "\nNamenode Configured Successfully at "+nip+"<br><hr>"
...
print "<br>JOBTREACKER configured successfully<br>"
...
""")
	fp.close()
	commands.getstatusoutput("sudo sshpass -p 123 scp /etc/cluster/tt.py "+ip+":/etc/cluster/")
	print commands.getstatusoutput("sudo sshpass -p 123 ssh -l root {} python /etc/cluster/tt.py".format(ip))[1]
	print "<br>"
	fpp = open("/etc/cluster/infocluster.txt","a+")
	fpp.write("tt "+ip+"\n")
	fpp.close()
...
""")
	fp.close()
	commands.getstatusoutput("sudo sshpass -p 123 scp /etc/cluster/dn.py "+ip+":/etc/cluster/dn.py")
	print "<br>"
	print commands.getstatusoutput("sudo sshpass -p 123 ssh -l root {} python /etc/cluster/dn.py".format(ip))[1]
	print "<hr><br>"
	fpp = open("/etc/cluster/infocluster.txt","a+")
	fpp.write("dn "+ip+"\n")
	fpp.close()
