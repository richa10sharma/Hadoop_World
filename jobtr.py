...
f = cgi.FieldStorage()
filename = f.getvalue('filen')
jobname = f.getvalue('jobn')
outfile = f.getvalue('out')
print jobname+" is the job to be performed<br>"
print "<br>"
print filename+" is the file to be worked upon"
#fs = open("/etc/cluster/infocluster.txt","r+")
#matter=fs.read()|
nip = commands.getstatusoutput("cat /etc/cluster/infocluster.txt | grep 'nn' | awk '{print $2}'")[1]
jip = commands.getstatusoutput("cat /etc/cluster/infocluster.txt | grep 'jt' | awk '{print $2}'")[1]
commands.getstatusoutput("sudo sshpass -p 123 ssh -l root "+jip+" hadoop jar /usr/share/hadoop/hadoop-examples-1.2.1.jar wordcount /"+filename+" /"+outfile)
print ("<br><br>words and their word count are<br><br>")
pr = commands.getstatusoutput("sudo sshpass -p 123 ssh -l root "+jip+" hadoop fs -cat /"+outfile+"/part-r-00000")[1]
print pr
